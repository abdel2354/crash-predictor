...
def GeT_Name(uid , Token):
    data = bytes.fromhex(EnC_AEs_sync(f"08{EnC_Uid_sync(uid , Tp = 'Uid')}1007"))
    ...
    if response.status_code in (200, 201):
        ...
        BesTo_data = json.loads(DeCode_PackEt_sync(packet))      
        try:
...
def DeLet_Uid(id , Tok):
    ...
    headers = {
        ...
        'Connection': 'Keep-Alive',
        'Accept-Encoding': 'gzip'}
    data = bytes.fromhex(EnC_AEs_sync(f"08a7c4839f1e10{EnC_Uid_sync(id , Tp = 'Uid')}"))
    ResPonse = requests.post(url , headers=headers , data=data , verify=False)    
