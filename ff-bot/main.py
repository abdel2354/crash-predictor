...
📱 Player will see:
"—͞ABHIᅠ wants to join your team"
...
[FFDD00]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[00FF00][b][c]BOT MADE BY ABHI GMR  
"""
        else:
...
[FFDD00]°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°°
[00FF00][b][c]BOT MADE BY ㅤABHI GMR
"""
        else:
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][DA70D6]❀️abhi:[FFFFFF]</> ABHI GMR
"""
...
async def api_add_friend(uid, password, friend_uid):
    """Add friend using uid+password via primary API"""
    try:
...
async def api_add_friend_by_token(jwt_token, friend_uid, server_name):
    """Add friend using JWT token"""
    try:
        url = f"https://mafuuuuu-add.vercel.app/adding_friend_by_token?jwt_token={jwt_token}&friend_uid={friend_uid}&server_name={server_name}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                if resp.status == 200:
                    data = await resp.text()
...
async def api_remove_friend(uid, password, friend_uid):
    """Remove friend using uid+password via primary API"""
    try:
...
async def api_remove_friend_by_token(jwt_token, friend_uid, server_name):
    """Remove friend using JWT token"""
    try:
        url = f"https://mafuuuuu-add.vercel.app/remove_friend_by_token?jwt_token={jwt_token}&friend_uid={friend_uid}&server_name={server_name}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                if resp.status == 200:
                    data = await resp.text()
...
async def api_guild_join(guild_id, guest_uid, guest_pw):
    """Join a guild using guest account"""
    try:
...
async def api_guild_leave(guild_id, guest_uid, guest_pw):
    """Leave a guild using guest account"""
    try:
...
[1E90FF]ＤＥＶ   [FF0000]   A BㅤH I G A M I N G
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FF0000]_ _ _ X ³ _ _ _ _  
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[9400D3]M A D E B Y [FF0000] ABHI 
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
[FFD700] ＦＯＬＬＯＷ    ＭＥ   ＩＮ   [87CEEB]TELEGRAM: [FF0000]@ABHI_GMR
[FF0000]━[FFFF00]━[0000FF]━[FFFF00]━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
    fields = {
    1: 5,
    2: {
...
[C][B][FFFF00]│[FF0000]▶[00FFFF]│[00FFFF]║[FFFFFF] ABHI TCP BOT[FFFF00]║
...
[b][c][00FF00]Wʟᴄ Tᴏ ABHI Bᴏᴛ Hᴇʀᴇ 
❀️ [B][C][00FFFF]╭[00FFFF]─[00FFFF]╮[00FFFF]╔═══════╗
[C][B][00FFFF]│[FFFFFF]ꚠ[00FFFF]│[00FFFF]║[FFFFFF]ㅤ@ABHI BOT[00FFFF]║
[C][B][00FFFF]╰[00FFFF]─[00FFFF]╯[00FFFF]╚═══════╝
[00FFFF]━━━━━━━━━━━━[FF69B4]"""
                        # In your auto-join (Old Handler) code, find this line:
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/addfriend '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /addfriend [uid]\nExample: /addfriend 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                friend_uid = parts[1]
                                if not friend_uid.isdigit() or len(friend_uid) < 8:
                                    error_msg = f"[B][C][FF0000]❌ Invalid UID! Must be 8+ digits\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]🤝 Adding friend {friend_uid}...\n", uid, chat_id, key, iv)
                                    try:
                                        credentials = load_credentials_from_file("bot.txt")
                                        if credentials:
                                            bot_uid, bot_pw = credentials
                                            success, result = await api_add_friend(bot_uid, bot_pw, friend_uid)
                                            if success:
                                                result_msg = f"[B][C][00FF00]✅ Friend request sent to {friend_uid}!\n[FFFFFF]{result}\n"
                                            else:
                                                result_msg = f"[B][C][FF0000]❌ Failed: {result}\n"
                                        else:
                                            result_msg = f"[B][C][FF0000]❌ Bot credentials not found!\n"
                                        await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
                                    except Exception as e:
                                        await safe_send_message(response.Data.chat_type, f"[B][C][FF0000]❌ Error: {str(e)[:50]}\n", uid, chat_id, key, iv)

                        # ======= NEW: /removefriend command =======
                        if inPuTMsG.strip().startswith('/removefriend '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /removefriend [uid]\nExample: /removefriend 123456789\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                friend_uid = parts[1]
                                if not friend_uid.isdigit() or len(friend_uid) < 8:
                                    ...
                                    await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]🗑️ Removing friend {friend_uid}...\n", uid, chat_id, key, iv)
                                    try:
                                        credentials = load_credentials_from_file("bot.txt")
                                        if credentials:
                                            bot_uid, bot_pw = credentials
                                            success, result = await api_remove_friend(bot_uid, bot_pw, friend_uid)
                                            if success:
                                                result_msg = f"[B][C][00FF00]✅ Friend {friend_uid} removed!\n[FFFFFF]{result}\n"
                                            else:
                                                result_msg = f"[B][C][FF0000]❌ Failed: {result}\n"
                                        else:
                                            result_msg = f"[B][C][FF0000]❌ Bot credentials not found!\n"
                        ...
                        # ======= NEW: /guildinfo command =======
                        if inPuTMsG.strip().startswith('/guildinfo '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /guildinfo [guild_id] [region]\nExample: /guildinfo 123456 IND\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                guild_id = parts[1]
                                guild_region = parts[2].upper() if len(parts) > 2 else region
                                await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]🏰 Fetching guild info for {guild_id}...\n", uid, chat_id, key, iv)
                                try:
                                    success, data = await api_guild_info(guild_id, guild_region)
                                    if success and isinstance(data, dict):
                                        guild_name = data.get('guild_name', data.get('name', 'Unknown'))
                                        guild_level = data.get('guild_level', data.get('level', 'N/A'))
                                        guild_members = data.get('guild_members', data.get('members', 'N/A'))
                                        guild_leader = data.get('guild_leader', data.get('leader', 'N/A'))
                                        guild_capacity = data.get('guild_capacity', data.get('capacity', 'N/A'))
                                        result_msg = f"""[B][C][00FFFF]━━━━━━━━━━━━━━━
[00FFFF] ■ GUILD INFO ■
[00FFFF]━━━━━━━━━━━━━━━━
[FFFFFF]│ Name: [FFFF00]{guild_name}
[FFFFFF]│ ID: [FFFF00]{guild_id}
[FFFFFF]│ Level: [FFFF00]{guild_level}
[FFFFFF]│ Members: [FFFF00]{guild_members}/{guild_capacity}
[FFFFFF]│ Leader: [FFFF00]{guild_leader}
[FFFFFF]│ Region: [FFFF00]{guild_region}
[00FFFF]━━━━━━━━━━━━━━━━"""
                                    elif success:
                                        result_msg = f"[B][C][00FF00]✅ Guild Info:\n[FFFFFF]{data}\n"
                                    else:
                                        result_msg = f"[B][C][FF0000]❌ Failed: {data}\n"
                                    await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
                                except Exception as e:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][FF0000]❌ Error: {str(e)[:50]}\n", uid, chat_id, key, iv)

                        # ======= NEW: /guildjoin command =======
                        if inPuTMsG.strip().startswith('/guildjoin '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF0000]❌ Usage: /guildjoin [guild_id] [uid] [password]\nExample: /guildjoin 123456 987654 mypass\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                guild_id = parts[1]
                                guest_uid = parts[2]
                                guest_pw = parts[3]
                                await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]🏰 Joining guild {guild_id}...\n", uid, chat_id, key, iv)
                                try:
                                    success, result = await api_guild_join(guild_id, guest_uid, guest_pw)
                                    if success:
                                        result_msg = f"[B][C][00FF00]✅ Joined guild {guild_id}!\n[FFFFFF]{result}\n"
                                    else:
                                        result_msg = f"[B][C][FF0000]❌ Failed: {result}\n"
                                    await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
                                except Exception as e:
                        ...
                        # ======= NEW: /guildleave command =======
                        if inPuTMsG.strip().startswith('/guildleave '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF0000]❌ Usage: /guildleave [guild_id] [uid] [password]\nExample: /guildleave 123456 987654 mypass\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                guild_id = parts[1]
                                guest_uid = parts[2]
                                guest_pw = parts[3]
                                await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]🏰 Leaving guild {guild_id}...\n", uid, chat_id, key, iv)
                                try:
                                    success, result = await api_guild_leave(guild_id, guest_uid, guest_pw)
                                    if success:
                                        result_msg = f"[B][C][00FF00]✅ Left guild {guild_id}!\n[FFFFFF]{result}\n"
                                    else:
                                        result_msg = f"[B][C][FF0000]❌ Failed: {result}\n"
                                    await safe_send_message(response.Data.chat_type, result_msg, uid, chat_id, key, iv)
                                except Exception as e:
...
[FF0000]━━━━━━━━━━━━━━━━━━
[FF0000]│
[FF0000]├ [FF0000]/3 [FFFFFF](3 Player Grp)
[FF0000]├ [FF0000]/5 [FFFFFF](5 Player Grp)
[FF0000]├ [FF0000]/6 [FFFFFF](6 Player Grp)
[FF0000]└ [FF0000]/inv [FFFFFF][uid]

[FF0000]━━━━━━━━━━━━━━━━━━

[FF0000] ■  MATCH CONTROL ■
[FF0000]━━━━━━━━━━━━━━━━━━
[FF0000]│
[FF0000]├ [00FF00]! [FFFFFF][code] [FFFFFF](Join)
[FF0000]├ [00FF00]/exit
[FF0000]├ [00FF00]/start
[FF0000]└ [00FF00]/kick [FFFFFF][uid]
...
[FF0000]━━━━━━━━━━━━━━━━━"""
                            ...
                            emotes = """[B][C][FF00FF]━━━━━━━━━━━━━━
[FF00FF] ■ FUN & ENTERTAINMENT ■
[FF00FF]━━━━━━━━━━━━━━━━━

[FF00FF]━━ GAMES & JOKES ━━
[FF00FF]│ [FF00FF]/love [FFFFFF][name]
[FF00FF]│ [FF00FF]/boka [FFFFFF][name]
[FF00FF]│ [FF00FF]/lu[FFFFFF]ck
[FF00FF]└ [FF00FF]/stic[FFFFFF]ker


[FF00FF]━━ FUN TEXTS ━━
[FF00FF]│ [FF00FF] no[FFFFFF]ob
[FF00FF]│ [FF00FF] /mg [FFFFFF][txt]
[FF00FF]└ [FF00FF]/msg [FFFFFF][text]
[FF00FF]━━━━━━━━━━━━━━━━━━"""
...
[00FFFF]━━ BASIC ━━
[00FFFF]│ /tt [FFFFFF][username]
[00FFFF]└ [00FFFF]@


[00FFFF]━━ CUSTOM & FAST ━━
[00FFFF]│ [00FFFF]/fast [FFFFFF][uids] [id]
[00FFFF]└ [00FFFF]/e  [FFFFFF][name]


[00FFFF]━━ CUSTOM & FAST ━━
[00FFFF]│ [00FFFF]/evo 1-379 [FFFFFF][nmbr]
[00FFFF]│ [00FFFF]/e [FFFFFF][tem_cd] [emt_id] [uid]
[00FFFF]└ [00FFFF]@[FFFFFF]s
[00FFFF]━━━━━━━━━━━━━━━━━━━━"""
...
[4DA6FF]│
[FFFFFF]├ Owner : [FFFF00]ㅤ—͞Hᴀsɪʙ Cᴏᴅᴇx
[FFFFFF]└ TG : [00FF00]@MG24_CODEX
...
[00FF88]━━ FRIEND SYSTEM ━━
[FF0000]│ [00FF00]/addfriend [FFFFFF][uid]
[FF0000]└ [00FF00]/removefriend [FFFFFF][uid]

[00FF88]━━ GUILD MANAGEMENT ━━
[FF0000]│ [00FF00]/guildinfo [FFFFFF][id] [region]
[FF0000]│ [00FF00]/guildjoin [FFFFFF][id] [uid] [pw]
[FF0000]└ [00FF00]/guildleave [FFFFFF][id] [uid] [pw]
[00FF88]━━━━━━━━━━━━━━━━━"""

                            await safe_send_message(response.Data.chat_type, new_cmnd, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 MG24 GAMERᅠ BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 MG24 GAMERᅠ BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 MG24 GAMERᅠ BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
